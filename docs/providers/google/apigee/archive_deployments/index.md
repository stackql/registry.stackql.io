...
| `organizations_environments_archiveDeployments_generateDownloadUrl` | `EXEC` | `archiveDeploymentsId:generateDownloadUrl, environmentsId, organizationsId` | Generates a signed URL for downloading the original zip file used to create an Archive Deployment. The URL is only valid for a limited period and should be used within minutes after generation. Each call returns a new upload URL. |
