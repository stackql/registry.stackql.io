...
|:-----|:--------------|:----------------|:------------|
| `networks_get` | `SELECT` | `network, project` | Returns the specified network. Gets a list of available networks by making a list() request. |
| `networks_list` | `SELECT` | `project` | Retrieves the list of networks available to the specified project. |
| `networks_insert` | `INSERT` | `project` | Creates a network in the specified project using the data included in the request. |
| `networks_delete` | `DELETE` | `network, project` | Deletes the specified network. |
| `networks_patch` | `EXEC` | `network, project` | Patches the specified network with the data included in the request. Only the following fields can be modified: routingConfig.routingMode. |
| `networks_switchToCustomMode` | `EXEC` | `network, project` | Switches the network mode from auto subnet mode to custom subnet mode. |
| `networks_updatePeering` | `EXEC` | `network, project` | Updates the specified network peering with the data included in the request. You can only modify the NetworkPeering.export_custom_routes field and the NetworkPeering.import_custom_routes field. |
