...
|:-----|:--------------|:----------------|:------------|
| `subnetworks_aggregatedList` | `SELECT` | `project` | Retrieves an aggregated list of subnetworks. |
| `subnetworks_get` | `SELECT` | `project, region, subnetwork` | Returns the specified subnetwork. Gets a list of available subnetworks list() request. |
| `subnetworks_list` | `SELECT` | `project, region` | Retrieves a list of subnetworks available to the specified project. |
| `subnetworks_insert` | `INSERT` | `project, region` | Creates a subnetwork in the specified project using the data included in the request. |
| `subnetworks_delete` | `DELETE` | `project, region, subnetwork` | Deletes the specified subnetwork. |
| `subnetworks_expandIpCidrRange` | `EXEC` | `project, region, subnetwork` | Expands the IP CIDR range of the subnetwork to a specified value. |
| `subnetworks_patch` | `EXEC` | `project, region, subnetwork` | Patches the specified subnetwork with the data included in the request. Only certain fields can be updated with a patch request as indicated in the field descriptions. You must specify the current fingerprint of the subnetwork resource being patched. |
| `subnetworks_setPrivateIpGoogleAccess` | `EXEC` | `project, region, subnetwork` | Set whether VMs in this subnet can access Google services without assigning external IP addresses through Private Google Access. |
