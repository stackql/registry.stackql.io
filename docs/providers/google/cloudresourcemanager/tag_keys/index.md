...
|:-----|:--------------|:----------------|:------------|
| `tagKeys_get` | `SELECT` | `name` | Retrieves a TagKey. This method will return `PERMISSION_DENIED` if the key does not exist or the user does not have permission to view it. |
...
| `tagKeys_delete` | `DELETE` | `name` | Deletes a TagKey. The TagKey cannot be deleted if it has any child TagValues. |
| `tagKeys_patch` | `EXEC` | `name` | Updates the attributes of the TagKey resource. |
